...
namespace CourierPortal.Api.Controllers.Portal
{
    ...
    public class ContractsController(PortalContractService contractService) : BaseController
    {
        // GET api/<controller>/Current/File
        [HttpGet("Current/File")]
        public async Task<IActionResult> GetCurrentFile([FromRoute] IdentifierRequest request)
        {
            try
            {
                ...
                return HandleResponse(await contractService.GetCurrentFile(GetCurrentUser(), messageId));
            }
            catch (Exception e)
            {
