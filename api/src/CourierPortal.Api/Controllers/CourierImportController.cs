...
/// <summary>
/// NP courier bulk import — upload, AI-map columns, validate, and execute.
/// </summary>
[ApiController]
[Route("api/v1/np/courier-import")]
[Produces("application/json")]
[Authorize(Policy = "NpAccess")]
public class CourierImportController : ControllerBase
{
    private readonly ICourierImportService _importService;
    private int NpAgentId => (int)(HttpContext.Items["NpAgentId"] ?? throw new UnauthorizedAccessException("NpAgentId not set."));

    public CourierImportController(ICourierImportService importService) => _importService = importService;
    ...
    [HttpPost("validate")]
    public async Task<IActionResult> Validate([FromBody] ValidateCourierImportRequest request)
    {
    ...
    /// <summary>Execute the import — creates courier records.</summary>
    [HttpPost("execute")]
    public async Task<IActionResult> Execute([FromBody] ExecuteCourierImportRequest request)
    {
        if (request.Rows is null || request.Rows.Count == 0)
            return BadRequest("No rows to import.");

        // We need the mapping to extract fields — it's stored in the request
        var mapping = new CourierColumnMappingDto(); // Client sends mapped data with field keys
